...
const Pagination = ({total, limit, page, sibling, navigate, location}) => {
    const paginations = usePagination({ total, limit, currentPage: page, sibling })
  ...
  return (
    <div className='flex items-center justify-center gap-2'>
      <Button 
        ...
        className={twMerge(clsx("bg-main-500", !page || +page === 1 ? 'opacity-50' : ''))}
      >
        <FaArrowLeft/>
      </Button>
      ...
      <Button 
        ...
        className={twMerge(
          clsx("bg-main-500", +page === Math.ceil(+total / +limit) ? 'opacity-50' : '')
        )}
      >
        <FaArrowRight/>
      </Button>
